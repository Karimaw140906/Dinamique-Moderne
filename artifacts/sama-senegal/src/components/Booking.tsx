...
export function Booking() {
  ...
  useEffect(() => {
    const load = () => {
      try {
        setToursData(
          JSON.parse(localStorage.getItem("adminTours") || "[]").filter(
            (i: any) => i.active,
          ),
        );
      } catch {}
      try {
        setTransportData(
          JSON.parse(localStorage.getItem("transportData") || "[]").filter(
            (i: any) => i.active,
          ),
        );
      } catch {}
      try {
        setRestaurantsData(
          JSON.parse(localStorage.getItem("restaurantsData") || "[]").filter(
            (i: any) => i.active,
          ),
        );
      } catch {}
      try {
        setHotelsData(
          JSON.parse(localStorage.getItem("hotelsData") || "[]").filter(
            (i: any) => i.active,
          ),
        );
      } catch {}
      try {
        setActivitiesData(
          JSON.parse(localStorage.getItem("activitiesData") || "[]").filter(
            (i: any) => i.active,
          ),
        );
      } catch {}
...
${data.message}`;
  ...
  return (
    <section
      id="reserver"
      className="py-24 bg-gradient-to-br from-primary to-foreground text-white"
    >
        ...
        <div className="max-w-4xl mx-auto bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 md:p-12 shadow-2xl">
          <div className="text-center mb-10">
            ...
            <p className="text-white/70">
              ...
              rapidement via WhatsApp pour confirmer votre visite.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <SelectContent className="max-h-[300px]">
                    {toursData.length > 0 && (
                      <SelectGroup>
                        ...
                        {toursData.map((t) => (
                          <SelectItem key={`tour-${t.id}`} value={t.name}>
                            {t.emoji} {t.name} - {t.price} FCFA
                          </SelectItem>
                        ))}
                      </SelectGroup>
                    )}
                    {transportData.length > 0 && (
                      <SelectGroup>
                        ...
                        {transportData.map((t) => (
                          <SelectItem key={`trans-${t.id}`} value={t.name}>
                            🚗 {t.name} - {t.priceDay} FCFA/j
                          </SelectItem>
                        ))}
                      </SelectGroup>
                    )}
                    {activitiesData.length > 0 && (
                      <SelectGroup>
                        ...
                        {activitiesData.map((a) => (
                          <SelectItem key={`act-${a.id}`} value={getName(a)}>
                            🎯 {getName(a)} - {a.price} FCFA
                          </SelectItem>
                        ))}
                      </SelectGroup>
                    )}
                    {restaurantsData.length > 0 && (
                      <SelectGroup>
                        ...
                        {restaurantsData.map((r) => (
                          <SelectItem key={`rest-${r.id}`} value={r.name}>
                            🍽️ {r.name}
                          </SelectItem>
                        ))}
                      </SelectGroup>
                    ...
                    {hotelsData.length > 0 && (
                      <SelectGroup>
                        ...
                        {hotelsData.map((h) => (
                          <SelectItem key={`hot-${h.id}`} value={h.name}>
                            🏨 {h.name} - {h.priceNight} FCFA/n
                          </SelectItem>
                        ))}
                      </SelectGroup>
                    )}
            ...
            <div className="pt-4">
              <Button
                type="submit"
                size="lg"
                className="w-full bg-[#25D366] hover:bg-[#20bd5a] text-white font-bold h-14 text-lg"
              >

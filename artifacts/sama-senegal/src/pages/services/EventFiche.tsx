...
export default function EventFiche() {
  ...
  return (
    <div className="min-h-screen bg-[#2B1B4D]">
      {ev.photo && <img src={ev.photo} alt={ev.name} className="w-full h-56 sm:h-72 md:h-96 lg:h-[30rem] object-cover" />}
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        <div className="bg-white rounded-2xl shadow-md p-6 space-y-4">
          <h1 className="text-3xl font-serif font-bold text-[#0B0A14]">{ev.name}</h1>
          <div className="flex flex-wrap gap-4 text-sm text-gray-500">
            {ev.date_start && (
              <div className="flex items-center gap-2"><Calendar className="w-4 h-4 text-[#F5B942]" /> {formatDate(ev.date_start)}</div>
            )}
            {ev.location && (
              <div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-[#F5B942]" /> {ev.location}</div>
            )}
          </div>
          ...
          <div className="flex justify-between items-center pt-4 border-t">
            <div>
              ...
              <div className="text-2xl font-bold text-[#F5B942]">{ev.price ? convertPrice(ev.price) : "Gratuit"}</div>
            </div>

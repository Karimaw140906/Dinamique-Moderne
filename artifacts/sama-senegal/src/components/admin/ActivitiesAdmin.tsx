...
const DEFAULT_DATA = [
  {"id":1,"nameFR":"Balade en Pirogue","nameEN":"Pirogue Ride","nameES":"Paseo en Piragua","photo":"","category":"Sport nautique","descFR":"Exploration des côtes en pirogue traditionnelle","descEN":"Coastal exploration by traditional pirogue","descES":"Exploración costera en piragua tradicional","duration":"2h","price":8000,"minParticipants":2,"location":"Île de Gorée","active":true},
  {"id":2,"nameFR":"Cours de Cuisine Sénégalaise","nameEN":"Senegalese Cooking Class","nameES":"Clase de Cocina Senegalesa","photo":"","category":"Culturel","descFR":"Apprenez à cuisiner le thiéboudienne","descEN":"Learn to cook thiéboudienne","descES":"Aprende a cocinar thiéboudienne","duration":"3h","price":12000,"minParticipants":1,"location":"Gorée","active":true}
];

const CATEGORIES = ["Sport nautique", "Culturel", "Bien-être", "Musique", "Art", "Aventure", "Enfants"];
...
export function ActivitiesAdmin() {
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" placeholder="ex: 2h" value={item.duration || ""} onChange={(e) => onChange("duration", e.target.value)} className="w-full mt-1 border rounded-lg p-2" />
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={item.location || ""} onChange={(e) => onChange("location", e.target.value)} className="w-full mt-1 border rounded-lg p-2" />
          </div>
          <div>
            <label className="text-xs font-bold text-gray-500 uppercase">Participants min.</label>
            <input type="number" min="1" value={item.minParticipants || 1} onChange={(e) => onChange("minParticipants", parseInt(e.target.value))} className="w-full mt-1 border rounded-lg p-2" />
          </div>
        </div>
  ...
  const renderCard = (item: any) => (
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-gray-500 mt-1">{item.category} • {item.duration}</div>
        <div className="text-sm font-bold text-[#D4A017] mt-1">{item.price} FCFA</div>
        <div className="mt-2">
  ...
  return (
    <CrudSection
      ...
      items={items}
      setItems={saveItems}
      defaultItem={{ nameFR: "", category: "Culturel", active: true, price: 0, minParticipants: 1, duration: "" }}

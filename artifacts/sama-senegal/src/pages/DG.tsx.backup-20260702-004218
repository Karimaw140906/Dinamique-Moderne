...
const CHART_COLORS = ["#2C7A5C", "#D4A017", "#C2622D", "#1A1A2E", "#5C3D1E", "#7A9E9F"];
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={() => setEditing(!editing)} className="text-gray-400 hover:text-[#2C7A5C]">
      ...
      {editing ? (
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2C7A5C]/30"
          />
          <button
            onClick={() => { onSave(def.key, def.label, draft); setEditing(false); }}
            className="p-2 bg-[#2C7A5C] text-white rounded-lg hover:bg-[#245f49]"
          >
...
export default function DG() {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const [b, p, mk] = await Promise.all([loadBookings(), loadPayments(), loadManualKpis()]);
      if (!mounted) return;
      setBookings(b);
      setPayments(p);
      setProviderRequests(loadProviderRequests());
      setManualKpis(mk);
      ...
      <header className="bg-gradient-to-r from-[#1A1A2E] to-[#2C3E50] text-white sticky top-0 z-40 shadow-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#D4A017] flex items-center justify-center text-xl font-black shadow-lg">🏛️</div>
            <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex gap-1 border-t border-white/10">
          <button onClick={() => setTab("pilotage")}
            className={`px-4 py-2.5 text-sm font-bold border-b-2 transition-colors ${tab === "pilotage" ? "border-[#D4A017] text-[#D4A017]" : "border-transparent text-white/50 hover:text-white"}`}>
            <LayoutDashboard className="w-4 h-4 inline mr-1.5 -mt-0.5" /> Pilotage
          </button>
          <button onClick={() => setTab("affichage")}
            className={`px-4 py-2.5 text-sm font-bold border-b-2 transition-colors ${tab === "affichage" ? "border-[#D4A017] text-[#D4A017]" : "border-transparent text-white/50 hover:text-white"}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-10">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-white/70 mt-1 text-sm">
                Vous avez <span className="text-[#D4A017] font-bold">{tactical.pendingBookings}</span> réservation(s) en attente
                {tactical.unreadMessages > 0 && <> et <span className="text-[#D4A017] font-bold">{tactical.unreadMessages}</span> message(s) non lu(s)</>}.
        ...
        <section>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <KpiCard icon={<CreditCard className="w-6 h-6 text-white" />} label="Chiffre d'affaires global" value={convertPrice(strategic.totalRevenue)} color="bg-[#2C7A5C]" trend="up" />
            <KpiCard icon={<TrendingUp className="w-6 h-6 text-white" />} label="CA du mois" value={convertPrice(strategic.monthlyRevenue)}
              ...
              color="bg-[#D4A017]" />
            <KpiCard icon={<BookOpen className="w-6 h-6 text-white" />} label="Réservations totales" value={String(strategic.totalBookings)} color="bg-[#1A1A2E]" trend="neutral" />
          ...
          <div className="grid lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-sm text-gray-700 mb-4 flex items-center gap-2"><BarChart3 className="w-4 h-4 text-[#2C7A5C]" /> Évolution mensuelle du CA</h3>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={evolutionChartData}>
                  ...
                  <Bar dataKey="CA" fill="#2C7A5C" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-sm text-gray-700 mb-4 flex items-center gap-2"><PieChartIcon className="w-4 h-4 text-[#D4A017]" /> Répartition des ventes par catégorie</h3>
            ...
            {strategic.topServices.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={strategic.topServices} layout="vertical" margin={{ left: 40 }}>
                  ...
                  <Tooltip formatter={(v: number) => convertPrice(v)} />
                  <Bar dataKey="revenue" fill="#D4A017" radius={[0, 6, 6, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KpiCard icon={<Briefcase className="w-6 h-6 text-white" />} label="Demandes prestataires" value={String(tactical.pendingProviderRequests)} color="bg-[#2C7A5C]" trend="neutral" />
            <KpiCard icon={<Bell className="w-6 h-6 text-white" />} label="Messages non lus" value={String(tactical.unreadMessages)} color="bg-[#1A1A2E]" trend="neutral" />
          </div>
          {(tactical.refundsCount === 0 && tactical.disputesCount === 0) && (
            <p className="text-xs text-gray-400 mt-3">
              ℹ️ Remboursements et litiges : le mécanisme existe côté paiements mais n'est relié à aucune interface de gestion pour l'instant — ces compteurs resteront à 0 tant que ça n'est pas branché.
            </p>
          )}
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            <KpiCard icon={<Calendar className="w-6 h-6 text-white" />} label="Réservations du jour" value={String(operational.todayBookings)} color="bg-[#2C7A5C]" trend="neutral" />
            <KpiCard icon={<MapPin className="w-6 h-6 text-white" />} label="Tours actifs" value={String(operational.activeTours)} color="bg-[#D4A017]" trend="neutral" />
            <KpiCard icon={<UserCheck className="w-6 h-6 text-white" />} label="Guides actifs" value={String(operational.activeGuides)} color="bg-[#1A1A2E]" trend="neutral" />
          ...
          <p className="text-xs text-gray-400 mt-3">
            ℹ️ "Actifs" reflète le statut au catalogue, pas une disponibilité en temps réel (aucun système de disponibilité par ressource n'existe actuellement, hormis un calendrier global de dates bloquées).
          </p>
        </section>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {recentBookings.map((b, i) => (
                      <tr key={i} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3 font-medium text-[#1A1A2E] truncate max-w-[100px]">{b.client_name || "—"}</td>
                        ...
                        <td className="px-4 py-3 text-right font-bold text-[#2C7A5C]">{b.amount ? convertPrice(b.amount) : "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>

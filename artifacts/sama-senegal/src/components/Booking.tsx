...
import { CalendarIcon, Send, Check, X } from "lucide-react";
    ...
    const servicesText =
      selectedServices.length > 0
        ? allServices
            ...
            .map(
              (s) =>
                `  • ${s.emoji || ""} ${s.name}${s.price ? ` - ${s.price} FCFA` : ""}`,
            )
            .join("\n")
        : "Non spécifié";
...
_Réservation reçue via sama-senegal.vercel.app_`;
    ...
    window.open(
      `https://wa.me/221774188107?text=${encodeURIComponent(text)}`,
      "_blank",
    );
    onClose();
  };
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-gradient-to-br from-primary to-foreground rounded-3xl shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className={cn(
                      "w-full bg-white/10 border-white/20 text-white hover:bg-white/20 hover:text-white h-11 justify-start",
                      !date && "text-white/50",
                    )}
                  >
          ...
          <div className="space-y-4">
            <label className="text-sm font-medium text-white/80">
              Services souhaités{" "}
              <span className="text-white/50">(sélection multiple)</span>
            </label>

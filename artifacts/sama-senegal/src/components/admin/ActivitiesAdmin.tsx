...
export function ActivitiesSection() {
  ...
  const loadActivities = () => {
    try {
      ...
      return saved ? JSON.parse(saved).filter((a: any) => a.active) : [];
    } catch {
      return [];
    }
  };

  const [activities, setActivities] = useState<any[]>(() => loadActivities());

  useEffect(() => {
    const onUpdate = () => setActivities(loadActivities());
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filtered.map((a) => {
            ...
            return (
              <div
                key={a.id}
                className="bg-[#F5F0E8] rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all flex flex-col"
              >
                {a.photo ? (
                  <img
                    src={a.photo}
                ...
                <div className="p-6 flex flex-col flex-1">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="bg-[#D4A017] text-white text-xs font-bold px-2 py-1 rounded-full whitespace-nowrap ml-2">
                      {a.category}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm text-gray-600 mb-6">
                    <div className="flex items-center gap-2 bg-white px-2 py-1.5 rounded border border-gray-200">
                      <Clock className="w-4 h-4 text-[#2C7A5C]" />{" "}
                      <span>
                        {a.duration} {t("activities_duration")}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 bg-white px-2 py-1.5 rounded border border-gray-200">
                      <Users className="w-4 h-4 text-[#2C7A5C]" />{" "}
                      <span>
                        Min {a.minParticipants}{" "}
                        {t("activities_min_participants")}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 bg-white px-2 py-1.5 rounded border border-gray-200 col-span-2">
                      <MapPin className="w-4 h-4 text-[#2C7A5C]" />{" "}
                      <span>{a.location}</span>
                    </div>
                  </div>

                  <div className="mt-auto flex justify-between items-center pt-4 border-t border-gray-200">
                    <div className="text-xl font-bold text-[#2C7A5C]">

              ...
              <div key={v.id} className="bg-[#F5F0E8] rounded-2xl overflow-hidden shadow-md hover:shadow-xl hover:-translate-y-2 transition-all flex flex-col">
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="flex flex-wrap gap-3 mb-6">
                    <div className="flex items-center gap-1 text-sm bg-white px-2 py-1 rounded border border-gray-200"><Users className="w-4 h-4 text-gray-500" /> {v.seats} {t("transport_seats")}</div>
                    {v.aircon && <div className="flex items-center gap-1 text-sm bg-white px-2 py-1 rounded border border-gray-200"><Snowflake className="w-4 h-4 text-blue-500" /> {t("transport_aircon")}</div>}
                    {(v.driver_included || v.driverIncluded) && <div className="flex items-center gap-1 text-sm bg-green-50 px-2 py-1 rounded border border-green-200 text-green-700"><CheckCircle2 className="w-4 h-4" /> {t("transport_driver")}</div>}
                  </div>
                  <div className="mt-auto">
                    ...
                    <button onClick={() => document.querySelector("#reserver")?.scrollIntoView({ behavior: "smooth" })} className="w-full bg-[#1A1A2E] hover:bg-[#2C7A5C] text-white py-3 rounded-xl font-bold transition-colors">
                      {t("transport_book")}
                    </button>
                  </div>
                </div>

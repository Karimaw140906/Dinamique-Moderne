...
export default function ActiviteFiche() {
  const [, params] = useRoute("/activites/:id");
  ...
  if (!item) return <div className="min-h-screen flex items-center justify-center">Activité introuvable.</div>;

  const name = language === "EN" ? (item.name_en || item.name_fr) : language === "ES" ? (item.name_es || item.name_fr) : item.name_fr;

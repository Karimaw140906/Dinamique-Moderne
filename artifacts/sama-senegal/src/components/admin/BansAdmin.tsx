...
export function BansAdmin() {
  ...
  const load = () => setClients(loadClients());
  ...
  const ban = (client: any) => {
    const r = reason[client.id] || "Violation des conditions d'utilisation";
    saveClients(loadClients().map(c => c.id === client.id ? { ...c, banned: true, ban_reason: r } : c));
    setReason(prev => ({ ...prev, [client.id]: "" }));
    load();
  };

  const unban = (id: string) => {
    saveClients(loadClients().map(c => c.id === id ? { ...c, banned: false, ban_reason: null } : c));
    load();
  };

  const allFiltered = clients
    .filter(c => filter === "all" ? true : filter === "banned" ? c.banned : !c.banned)
    .filter(c => `${c.firstName || ""} ${c.lastName || ""} ${c.whatsapp || ""} ${c.email || ""}`.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">{clients.filter(c => c.banned).length} client(s) banni(s) · {clients.length} au total</p>
        </div>
        <div className="flex gap-2">
          {(["all", "active", "banned"] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {f === "all" ? `Tous (${clients.length})` : f === "active" ? `Actifs (${clients.filter(c => !c.banned).length})` : `Bannis (${clients.filter(c => c.banned).length})`}
            </button>
          ))}
        </div>
      </div>

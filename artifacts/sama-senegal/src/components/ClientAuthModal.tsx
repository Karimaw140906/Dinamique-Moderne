...
export function ClientAuthModal() {
  const { showClientModal, setShowClientModal, clientLogin, clientRegister } = useClientAuth();
  ...
  const texts = {
    FR: {
      ...
      email: "Email ou WhatsApp", pass: "Mot de passe", phone: "Numéro de téléphone",
      wa: "Numéro WhatsApp", sendCode: "Envoyer le code", verifyCode: "Code de vérification",
      ...
      errLogin: "Identifiants incorrects. Vérifiez votre email/WhatsApp et mot de passe.",
      ...
      byEmail: "Email / Mot de passe", byPhone: "Téléphone", byWa: "WhatsApp",
    },
    EN: {
      title: "My Account", login: "Sign In", register: "Create Account",
      email: "Email or WhatsApp", pass: "Password", phone: "Phone number",
      wa: "WhatsApp number", sendCode: "Send code", verifyCode: "Verification code",
      ...
      errLogin: "Incorrect credentials. Check your email/WhatsApp and password.",
      ...
      byEmail: "Email / Password", byPhone: "Phone", byWa: "WhatsApp",
    },
    ES: {
      title: "Mi Cuenta", login: "Iniciar sesión", register: "Crear cuenta",
      email: "Email o WhatsApp", pass: "Contraseña", phone: "Número de teléfono",
      wa: "Número de WhatsApp", sendCode: "Enviar código", verifyCode: "Código de verificación",
      ...
      errLogin: "Credenciales incorrectas. Verifica tu email/WhatsApp y contraseña.",
      ...
      byEmail: "Email / Contraseña", byPhone: "Teléfono", byWa: "WhatsApp",
    },
  };
  ...
  const handleLogin = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      const ok = clientLogin(loginId, loginPass);
      setLoading(false);
      if (ok) setShowClientModal(false);
      else setError(T.errLogin);
    }, 400);
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-gradient-to-r from-[#2C7A5C] to-[#1A1A2E] p-6 text-white flex justify-between items-center">
          ...
          <button onClick={() => setShowClientModal(false)} className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
            ...
            <div className="space-y-4">
              <div className="flex rounded-xl border border-gray-200 overflow-hidden text-xs">
                {(["email", "phone", "whatsapp"] as LoginMethod[]).map((m) => (
                  <button
                    key={m}
                    onClick={() => { setLoginMethod(m); setError(""); setCodeSent(false); }}
                ...
                <>
                  <div className="relative">
                    ...
                    <input type="text" value={loginId} onChange={(e) => setLoginId(e.target.value)}
                      placeholder={T.email} className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2C7A5C]/30" />
                  </div>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    <input type={showPassword ? "text" : "password"} value={loginPass} onChange={(e) => setLoginPass(e.target.value)}
                      placeholder={T.pass} className="w-full pl-10 pr-10 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2C7A5C]/30" />
              ...
              {loginMethod === "phone" && (
                <div className="relative">
                  ...
                  <input type="tel" value={loginId} onChange={(e) => setLoginId(e.target.value)}
                    placeholder={T.phone} className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2C7A5C]/30" />
                </div>
              )}
              {loginMethod === "whatsapp" && (
                <>
                  <div className="relative">
                    ...
                    <input type="tel" value={loginId} onChange={(e) => setLoginId(e.target.value)}
                      placeholder={T.wa} className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2C7A5C]/30" />
                  </div>
                  {codeSent && (
                    <input type="text" value={whatsappCode} onChange={(e) => setWhatsappCode(e.target.value)}
                      placeholder={T.verifyCode} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2C7A5C]/30" />
                  )}
                </>
              )}

              {error && <p className={`text-sm px-3 py-2 rounded-lg ${error === T.codeSentMsg ? "text-[#2C7A5C] bg-[#2C7A5C]/10" : "text-red-600 bg-red-50"}`}>{error}</p>}

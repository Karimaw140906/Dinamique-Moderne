    ...
    <div className="min-h-screen bg-[#2B1B4D]">
      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        <div className="bg-white rounded-2xl shadow-md p-6 space-y-4">
          ...
          {item.cuisine && <div className="text-xs font-bold text-[#6C3EF5] uppercase tracking-wide">{item.cuisine}</div>}
          {desc && <p className="text-gray-600">{desc}</p>}
          {item.address && (
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <MapPin className="w-4 h-4 text-[#F5B942]" /> {item.address}
            </div>
          )}
          {item.hours && (
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <Clock className="w-4 h-4 text-[#F5B942]" /> {item.hours}
            </div>
          )}
          {item.whatsapp && (
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <Phone className="w-4 h-4 text-[#F5B942]" /> {item.whatsapp}
            </div>
          )}
            ...
            <div>
              {item.price_range && (
                <>
                  ...
                  <div className="text-xl font-bold text-[#F5B942]">{item.price_range}</div>
                </>
              )}
            </div>

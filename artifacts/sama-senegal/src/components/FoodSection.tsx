...
import { useSupabaseData, DEFAULT_MENU } from "@/lib/useSupabaseData";
...
export function FoodSection() {
  ...
  const updateQuantity = (id: number, delta: number) => {
    setCart(prev =>
      prev.map(p => p.item.id === id ? { ...p, quantity: p.quantity + delta } : p)
  ...
  const orderWhatsApp = () => {
    ...
    const foodNum = "221774188107".replace(/[^0-9]/g, "");
                ...
                <div key={item.id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md border border-gray-100 transition-all duration-300 [transition-timing-function:var(--ease-premium)] flex flex-col zoom-on-hover">
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <div className="flex gap-2 mb-2">
                      <span className="text-[10px] bg-gray-100 px-2 py-0.5 rounded text-gray-600">{item.spice_level || item.spiceLevel}</span>
                      <span className="text-[10px] bg-gray-100 px-2 py-0.5 rounded text-gray-600">⏱️ {item.prep_time || item.prepTime} min</span>
                    </div>
                    ...
                    {cart.map(p => (
                      <div key={p.item.id} className="flex justify-between items-center text-sm">
                        <div className="flex-1 min-w-0 pr-2">
                          <div className="font-bold truncate">{language === "EN" ? (p.item.name_en || p.item.nameEN) : language === "ES" ? (p.item.name_es || p.item.nameES) : (p.item.name_fr || p.item.nameFR)}</div>

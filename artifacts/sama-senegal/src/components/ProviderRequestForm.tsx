...
function saveProviderRequest(data: any) {
  const record = { ...data, id: Date.now(), status: "pending", created_at: new Date().toISOString() };
  try {
  ...
  // Sync vers Supabase pour que la demande soit visible par le DG/Admin
  // depuis n'importe quel appareil (le localStorage seul ne suffit pas).
  const KNOWN_KEYS = new Set(["name", "email", "whatsapp", "other_role", "roles", "id", "status", "created_at"]);
  const documents: Record<string, string> = {};
  Object.keys(record).forEach((k) => { if (!KNOWN_KEYS.has(k)) documents[k] = (record as any)[k]; });
  ...
  (async () => {
    try {
      await supabase.from("provider_requests").insert({
        ...
        roles: record.roles,
        other_role: record.other_role || null,
        documents,
        status: "pending",
...
export function ProviderRequestForm({ onClose }: Props) {
  ...
  const [form, setForm] = useState({ name: "", email: "", whatsapp: "", other_role: "", roles: [] as string[] });
  ...
  const handleFileUpload = (key: string, file: File) => {
    ...
    reader.onload = (e) => setDocs(d => ({ ...d, [key]: e.target?.result as string }));
  ...
  const handleCGPAccept = () => {
    setShowCGP(false);
    setLoading(true);
    saveProviderRequest({ ...form, ...docs });
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="flex h-1">
          <div className="h-full bg-[#6C3EF5] transition-all duration-300" style={{ width: `${step === 1 ? 50 : 100}%` }} />
            ...
            <>
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                  ...
                  className="w-full mt-1 px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#6C3EF5]/30" />
              </div>
              <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {ROLES.map(r => (
                    <button key={r.value} type="button" onClick={() => toggleRole(r.value)}
                      className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-medium transition-all text-left ${form.roles.includes(r.value) ? "bg-[#6C3EF5] text-white border-[#6C3EF5]" : "bg-white text-gray-600 border-gray-200 hover:border-[#6C3EF5]/30"}`}>
                      <span>{r.icon}</span>
                ...
                {form.roles.includes("autre") && (
                  <input type="text" value={form.other_role} onChange={e => setForm({ ...form, other_role: e.target.value })}
                    ...
                    className="w-full mt-2 px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#6C3EF5]/30" />
                )}
              </div>
              ...
              <button onClick={() => { if (!form.name || !form.whatsapp || !form.email || form.roles.length === 0) { setError("Veuillez remplir tous les champs obligatoires (dont l'email) et choisir au moins un rôle."); return; } setError(""); setStep(2); }}
                className="w-full py-3 bg-[#F5B942] hover:bg-[#c49015] text-white font-bold rounded-xl transition-colors">
                Continuer →
              </button>
            </>
          )}
          ...
          {step === 2 && (
            <>
              ...
              {requiredDocs.length > 0 && (
                <div>
                  <h4 className="font-bold text-gray-800 mb-3 flex items-center gap-2"><span className="text-red-500">📌</span> Documents obligatoires</h4>
                  <div className="space-y-3">
                    {requiredDocs.map(doc => (
                      <div key={doc.key}>
                        <label className="text-xs font-bold text-gray-600 flex items-center gap-1">📌 {doc.label} <span className="text-red-500">*</span></label>
                        <div className="mt-1 flex items-center gap-3">
                          {docs[doc.key] ? (
                            <div className="flex items-center gap-2 text-green-600 text-sm bg-green-50 px-3 py-2 rounded-lg flex-1"><Check className="w-4 h-4" /> Document chargé</div>
                          ) : (
                            <label className="flex-1 cursor-pointer">
                              <div className="border-2 border-dashed border-gray-300 hover:border-[#6C3EF5] rounded-lg px-3 py-2 text-sm text-gray-500 hover:text-[#6C3EF5] flex items-center gap-2 transition-colors">
                          ...
                          {docs[doc.key] && <button onClick={() => setDocs(d => { const nd = { ...d }; delete nd[doc.key]; return nd; })} className="text-red-400 hover:text-red-600 text-xs">Supprimer</button>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {optionalDocs.length > 0 && (
                <div>
                  <h4 className="font-bold text-gray-800 mb-3 flex items-center gap-2"><span>⭐</span> Documents appréciés <span className="text-xs text-gray-400 font-normal">(augmentent vos chances)</span></h4>
                  <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-700 mb-3">💡 Ces documents renforcent votre crédibilité.</div>
                  <div className="space-y-3">
